...
function buildMonthGrid(year: number, month: number): CalendarDay[] {
  ...
  for (let i = 0; i < weekCount * 7; i++) {
    ...
    days.push({ date, iso: isoDate(date), inMonth: date.getMonth() === month });
  }
  return days;
}
...
export function CalendarView() {
  ...
  if (!semester) {
    return <NoSemesterNotice message="Create a semester to see deadlines and classes on a calendar." />;
  }
              ...
              >
                ...
                {meetings.map(({ course, meeting }, i) => (
                  <div
                    key={`${course.id}-${i}`}
                    className="cal-class"
                    style={cssVars({ "--cal-class-color": course.color })}
                    title={`${course.name} · ${meeting.start}–${meeting.end}${meeting.location ? ` · ${meeting.location}` : ""}`}
                  >
                    {meeting.start} {courseShortLabel(course)}
                  </div>
                ))}
                {dayTasks.map((task) => {
                  ...
                  return (
                    <button
                      ...
                      style={cssVars({ "--cal-task-color": course?.color ?? "var(--text-3)" })}
                      title={`${task.title}${course ? ` · ${courseShortLabel(course)}` : ""}`}
                      onClick={(e) => {
                        e.stopPropagation();
                        setModal({ task });
                      }}
