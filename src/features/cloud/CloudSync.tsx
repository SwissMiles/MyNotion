...
import { migrateStoredState } from "../../store/persistence";
import type { AppState } from "../../types";

async function fetchCloudState(db: SupabaseClient): Promise<unknown> {
...
export function CloudSync() {
  ...
  useEffect(() => {
    ...
    async function loadOnce() {
      try {
        const remote = migrateStoredState(await fetchCloudState(db!));
        if (cancelled) return;
        if (remote) {
