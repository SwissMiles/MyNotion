...
export function CourseGrades({ course }: { course: Course }) {
  ...
  return (
    <div>
      ...
      {cg.length > 0 && totalWeight < 100 && <TargetHint grades={cg} />}
...
function TargetHint({ grades }: { grades: GradeEntry[] }) {
  ...
  if (pass.needed > 6) {
    text = (
      <>A {PASS_GRADE.toFixed(1)} is out of reach — it would take a {pass.needed.toFixed(2)} average on the remaining {pass.remaining}%.</>
    );
  } else if (pass.needed <= 1) {
    const five = requiredGrade(grades, 5);
    text = (
      <>
        Your pass is secured — even a 1.0 on the remaining {pass.remaining}% keeps you at {PASS_GRADE.toFixed(1)} or better.
    ...
    text = (
      <>To pass with a {PASS_GRADE.toFixed(1)}, you need a {pass.needed.toFixed(2)} average on the remaining {pass.remaining}% of the weight.</>
    );
  }
